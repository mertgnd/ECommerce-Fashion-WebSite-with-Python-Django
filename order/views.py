...
def success(request):
    ...
    try:
        ...
        current_user = request.user
        ordercount = Order.objects.filter(user__id=current_user.id).count()
        order = Order.objects.filter(user__id=current_user.id).reverse()[ordercount]
        order.odeme = True
        order.save()
        print("except")
    ...
    ordermail = Order.objects.filter(user__id=current_user.id).reverse()[ordercount - 1]
